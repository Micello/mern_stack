...
export default function Card({ id, rank, suit, angle, height, scale }) {
    ...
    const handleMouseEnter = rank ? (e) => { e.currentTarget.style.transform = `rotate(${safeAngle}rad) translateY(${safeHeight * 10 - 40}px) translateX(${safeAngle * -900}px)`} : 
    ...
    return (
      <div
        ...
        className={`max-w-[125px] max-h-[200px] h-full rounded-md  relative focus:ring-violet-300 active:bg-violet-700 bg-[lightgrey] hover:bg-gray-50  box-border   border-2 border-solid border-[black] transition-transform duration-300 ease-in-out`}
